...
export const isStatusDagpengerOrAAP = (status: string): boolean => (statuserSomStotterDagpengerEllerAAP.some((s) => s === status));

export const isStatusTilstotendeYtelse = (status: string): boolean => (statuserSomStotterTilstottendeYtelser.some((s) => s === status));

export const isStatusFrilanserOrKombinasjon = (status: string): boolean => (statuserSomStotterFrilanser.some((s) => s === status));

export const isStatusArbeidstakerOrKombinasjon = (status: string): boolean => (statuserSomStotterArbeidstaker.some((s) => s === status));

export const isStatusSNOrKombinasjon = (status: string): boolean => (statuserSomStotterSelvstendigNaeringsdrivende.some((s) => s === status));

...
import { buildInitialValues, lagAktivitetFieldId, skalVurdereAktivitet, transformValues } from './VurderAktiviteterTabell';
...
describe('<VurderAktiviteterTabell>', () => {
  ...
  it('skal bygge initial values', () => {
    ...
    expect(initialValues[id1].beregningAktivitetNavn)
      .toBe('Arbeidsgiveren (384723894723)');
    expect(initialValues[id1].fom)
      .toBe('2019-01-01');
    expect(initialValues[id1].tom)
      .toBe(null);
    expect(initialValues[id1].skalBrukes)
      .toBe(null);

    expect(initialValues[id2].beregningAktivitetNavn)
      .toBe('Arbeidsgiveren2 (334534623342)...3456');
    expect(initialValues[id2].fom)
      .toBe('2019-01-01');
    expect(initialValues[id2].tom)
      .toBe('2019-02-02');
    expect(initialValues[id2].skalBrukes)
      .toBe("true");

    expect(initialValues[id3].beregningAktivitetNavn)
      .toBe('Arbeidsgiveren3 (01.01.1960)...6789');
    expect(initialValues[id3].fom)
      .toBe('2019-01-01');
    expect(initialValues[id3].tom)
      .toBe('2019-02-02');
    expect(initialValues[id3].skalBrukes)
      .toBe("false");

    expect(initialValues[idAAP].beregningAktivitetNavn)
      .toBe('Arbeidsavklaringspenger');
    expect(initialValues[idAAP].fom)
      .toBe('2019-01-01');
    expect(initialValues[idAAP].tom)
      .toBe('2020-02-02');
    expect(initialValues[idAAP].skalBrukes)
      .toBe("true");
  });
  ...
  it('skal transform values', () => {
    ...
    expect(transformed.length)
      .toBe(2);
    expect(transformed[0].oppdragsgiverOrg)
      .toBe('334534623342');
    expect(transformed[0].arbeidsforholdRef)
      .toBe(aktivitet2.arbeidsforholdId);
    expect(transformed[0].fom)
      .toBe('2019-01-01');
    expect(transformed[0].tom)
      .toBe('2019-02-02');
    expect(transformed[0].arbeidsgiverIdentifikator)
      .toBe('334534623342');
    expect(transformed[0].skalBrukes)
      .toBe(false);

    expect(transformed[1].oppdragsgiverOrg)
      .toBe(null);
    expect(transformed[1].arbeidsforholdRef)
      .toBe(aktivitet3.arbeidsforholdId);
    expect(transformed[1].fom)
      .toBe('2019-01-01');
    expect(transformed[1].tom)
      .toBe('2019-02-02');
    expect(transformed[1].arbeidsgiverIdentifikator)
      .toBe('324234234234');
    expect(transformed[1].skalBrukes)
      .toBe(false);
  });
